...
import { getCollages } from '../services/collageService';
import Collage from '../types/Collage';
import useFetch from '../hooks/useFetch';

const CollageContext = createContext<any>(null);

export const CollageProvider = ({ isAdmin = false, children } : { isAdmin?: boolean, children: React.ReactNode }) => {
    ...
    const { pending, error, data: fetchedCollages } = useFetch<Collage[]>(fetchCollages);

    const filterCollages = useCallback((collages: Collage[]): Collage[] => {
        if (!showSearchOptions) {
            return collages;
        }

        let filteredCollages: Collage[] = collages.filter((collage) =>
            collage.name.toLowerCase().trim().includes(searchValue.toLowerCase().trim())
        );

        if (sortedBy === "recent") {
            filteredCollages = filteredCollages.sort((a: Collage, b: Collage) => b.date.getTime() - a.date.getTime());
        } else if (sortedBy === "oldest") {
            filteredCollages = filteredCollages.sort((a, b) => a.date.getTime() - b.date.getTime());
        } else if (sortedBy === "name") {
            filteredCollages = filteredCollages.sort((a, b) => a.name.localeCompare(b.name));
        }
        ...
        if (groupBy === "date") {
            ...
            filteredCollages = Object.values(groupedByDate).flatMap((collages) => collages);
    ...
    }, [searchValue, sortedBy, groupBy, showSearchOptions]);
    ...
    useEffect(() => {
        const handler = setTimeout(() => {
            if (showSearchOptions && fetchedCollages && fetchedCollages.length > 0) {
    ...
    return (
        <CollageContext.Provider value={{ collages, pending, error }}>
            {children}
